        ...
        for (j = 2; j < disco[disco[end].inode.endDireto[i]].dir.TL; j++)
            if (!dirVazio(disco[disco[end].inode.endDireto[i]])) {
                ...
                if (disco[disco[disco[end].inode.endDireto[i]].dir.arquivo[j].endInode].inode.permissao[0] == 'd') {
                    // Se diretório
                    printf(AZUL);
                } else if (disco[disco[disco[end].inode.endDireto[i]].dir.arquivo[j].endInode].inode.permissao[0] ==
                           'l') {
                    // Se link
                    printf(CIANO);
                }
                ...
                while (j < QTDE_INODE_INDIRETO && disco[endSimples].inodeIndireto.endInd[j] != endNulo()) {
                    for (k = 2; k < disco[disco[endSimples].inodeIndireto.endInd[j]].dir.TL; k++)
                        if (!dirVazio(disco[disco[endSimples].inodeIndireto.endInd[j]])) {
                            if (disco[disco[disco[endSimples].inodeIndireto.endInd[j]].dir.arquivo[j].endInode].inode.
                                permissao[0] == 'd') {
                                printf(AZUL);
                            } else if (disco[disco[disco[endSimples].inodeIndireto.endInd[j]].dir.arquivo[j].endInode].
                                       inode.permissao[0] == 'l') {
                                printf(CIANO);
                            }
                            printf("%s     ",
                                ...
                                if (disco[disco[endSimples].inodeIndireto.endInd[k]].inode.permissao[0] != '\0')
                                    ...
                                    for (l = 2; l < disco[disco[endSimples].inodeIndireto.endInd[k]].dir.TL; l++)
                                        if (!dirVazio(disco[endSimples])) {
                                            ...
                                            if (disco[disco[disco[endSimples].inodeIndireto.endInd[k]].dir.arquivo[k].
                                                    endInode].inode.permissao[0] == 'd') {
                                                // Diretório
                                                printf(AZUL);
                                            } else if (disco[disco[disco[endSimples].inodeIndireto.endInd[k]].dir.
                                                           arquivo[k].endInode].inode.permissao[0] == 'l') {
                                                // Link
                                                printf(CIANO);
                                            }
                                            printf("%s     ",
        ...
        for (j = 2; j < disco[disco[end].inode.endDireto[i]].dir.TL; j++)
            if (!dirVazio(disco[disco[end].inode.endDireto[i]])) {
                ...
                printf("%s\t%d\t%s\t%s\t%s\t%s%s%s",
                       ...
                       disco[endInode].inode.ultimaAlteracao, cor,
                       disco[disco[end].inode.endDireto[i]].dir.arquivo[j].nome, RESET);
                    ...
                    for (k = 2; k < disco[disco[endSimples].inodeIndireto.endInd[j]].dir.TL; k++)
                        if (!dirVazio(disco[disco[endSimples].inodeIndireto.endInd[j]])) {
                            ...
                            printf("%s\t%d\t%s\t%s\t%s\t%s%s%s",
                                   ...
                                   disco[endInode].inode.proprietario, disco[endInode].inode.grupo,
                                   disco[endInode].inode.ultimaAlteracao, cor,
                                   disco[disco[endSimples].inodeIndireto.endInd[j]].dir.arquivo[k].nome, RESET);
...
void diminuirBlocos(Bloco *disco, int endInode, int *quant) {
    int i, j, k, endSimples, endDuplo, endTriplo, ultimoEnd;

    i = QTDE_INODE_INDIRETO - 1;
    endTriplo = disco[endInode].inode.endTriploIndireto;
    if (endTriplo != endNulo() && !bad(disco[endTriplo])) {
        ultimoEnd = disco[disco[disco[endTriplo].inodeIndireto.endInd[4]].inodeIndireto.endInd[4]].inodeIndireto.endInd[
            4];
        if (ultimoEnd != endNulo() /*&& ultimoEnd != 0*/) {
            diminuirBlocos(disco, ultimoEnd, &*quant);
            pushBlocoLivre(disco, ultimoEnd);
            inicializarBloco(&disco[ultimoEnd]);
        }
    }
    if (endTriplo != endNulo() && !bad(disco[endTriplo])) {
        while (i >= 0 && *quant > 0) {
            if (disco[endTriplo].inodeIndireto.endInd[i] != endNulo()) {
                ...
                endDuplo = disco[endTriplo].inodeIndireto.endInd[i];
                if (endDuplo != endNulo() && !bad(disco[endDuplo])) {
                    while (j >= 0 && *quant > 0) {
                        if (disco[endDuplo].inodeIndireto.endInd[j] != endNulo()) {
                            k = QTDE_INODE_INDIRETO - 1;
                            endSimples = disco[endDuplo].inodeIndireto.endInd[j];
                            if (endSimples != endNulo() && !bad(disco[endSimples])) {
                                while (k >= 0 && *quant > 0) {
    ...
    if (*quant > 0) {
        ...
        if (*quant > 0) {
            ...
            if (endSimples != endNulo() && !bad(disco[endSimples])) {
                while (i >= 0 && *quant > 0) {
                    if (disco[endSimples].inodeIndireto.endInd[i] != endNulo()) {
            ...
            if (*quant > 0) {
                ...
                if (!bad(disco[disco[endInode].inode.endDireto[i]])) {
                    while (i >= 0 && quant > 0) {
                        if (disco[endInode].inode.endDireto[i] != endNulo()) {
...
void rm(Bloco *disco, int endEntrada, char *nomeArq) {
    ...
    if (buscaArquivo(disco, endEntrada, nomeArq, &pos, &end) == -1)
        ...
        if (disco[inodeArq].inode.permissao[0] == 'd') {
            ...
            if (disco[endExcluir].bad == 1)
                printf("rm: falhou em remover '%s': Arquivo corrompido\n", nomeArq);
            else {
...
void removerLinkFisico(Bloco *disco, int raiz, char *usuario, int endDir, char *destino) {
    int i = 0, j = 0, k, endDestino, pos, endEntrada, inode, quant = 1;
    ...
    if (endDestino != -1) {
        if (buscaArquivo(disco, endDestino, nomeArquivo, &pos, &endEntrada)) {
            ...
            if (disco[inode].inode.contadorLink > 1) {
                ...
                diminuirBlocos(disco, inode, &quant);
